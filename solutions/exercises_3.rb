...
def people_who_can_buy_beer(people)
  people.select {|p| can_buy_beer?(p)}
end
...
def names_of_people_who_passed_english(results_set)
  results_set.select {|name, results| passed_english?(results)}.keys
end
...
def names_of_people_who_took_french(results_set)
  results_set.select {|name, results| !results['french'].nil?}.keys
end
...
def names_of_people_better_at_english_than_maths(results_set)
  results_set.select {|name, results| results['english'] > results['maths']}.keys
end
...
def names_of_departments_with_one_employee(departments)
  departments.select {|name, employees| employees.size == 1}.keys
end
...
def names_of_departments_with_employees_from_portsmouth(departments)
  departments.select {|name, employees| employees.any? {|e| e['hometown'] == 'Portsmouth'}}.keys
end
...
def names_of_departments_with_no_employees_from_outside_southampton(departments)
  departments.select {|name, employees| employees.all? {|e| e['hometown'] == 'Southampton'}}.keys
end
...
def names_of_departments_with_at_least_one_contractor(departments)
  departments.select {|name, employees| employees.any? {|e| e['status'] == 'contractor'}}.keys
end
...
def names_of_departments_with_more_contractors_than_permanent(departments)
  departments.select do |name, employees|
    employees.count {|e| e['status'] == 'contractor'} > employees.count {|e| e['status'] == 'permanent'}
  end.keys
end
